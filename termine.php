        ...
        <main class="content">
            ...
            <div class="termine-liste">
                <?php foreach ($termine as $t): ?>
                    <div class="news-card" style="border-left: 8px solid <?= ($t['typ'] === 'spiel') ? '#e67e22' : '#3498db' ?>;">
                        
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap;">
                            
                            <div style="background: #f4f7f6; padding: 15px; border-radius: 15px; text-align: center; min-width: 100px;">
                                <span style="display:block; font-size: 1.2rem; font-weight: bold; color: var(--secondary-blue);">
                                    <?= date("d.m.", strtotime($t['termin_datum'])) ?>
                                </span>
                                <span style="font-size: 0.9rem; color: #666;"><?= date("Y", strtotime($t['termin_datum'])) ?></span>
                            ...
                            <div style="flex: 1; padding: 0 20px;">
                                <?php if ($t['typ'] === 'spiel'): ?>
                                    ...
                                    <div style="margin-top: 10px; font-size: 0.9rem;">
                                        ...
                                        <?php else: ?>
                                            ...
                                            <a href="https://www.google.com/maps/search/?api=1&query=<?= urlencode($t['strasse'] . " " . $t['gegner_ort']) ?>" target="_blank" style="color: #3498db; margin-left: 10px;">
                                ...
                                <?php else: ?>
                                    <h2 style="margin-bottom: 5px;"><?= htmlspecialchars($t['titel'] ?: $t['veranstaltungsart']) ?></h2>
                            ...
                            <?php if ($t['typ'] === 'spiel'): ?>
                                <button onclick="toggleAufstellung(<?= $t['id'] ?>)" class="read-more" style="white-space: nowrap;">
                        ...
                        <?php if ($t['typ'] === 'spiel'): ?>
                            <div id="aufstellung-<?= $t['id'] ?>" style="display:none; margin-top: 30px; padding-top: 20px; border-top: 2px dashed #eee;">
                                ...
                                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px;">
                                    ...
                                    foreach ($kader as $gruppe => $ids): 
                                        foreach ($ids as $s_id): 
                                            ...
                                            if (!$s) continue;
                                    ?>
                                        <div style="text-align: center; background: white; padding: 10px; border-radius: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); border: 1px solid #eee;">
                                            <div class="profile-preview-circle" style="width: 70px; height: 70px; margin: 0 auto 10px; position: relative;">
                                                <img src="<?= getProfilbild($s['profilbild']) ?>" alt="Spieler">
                                                <?php if($s_id == $t['spielfuehrer_id']): ?>
                                                    <div title="Spielführer" style="position: absolute; bottom: -5px; right: -5px; background: #f1c40f; width: 25px; height: 25px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid white;">
                                                        <i class="fa-solid fa-star" style="color: white; font-size: 0.7rem;"></i>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                            <div style="font-weight: bold; font-size: 0.85rem;"><?= htmlspecialchars($s['vorname']) ?></div>
